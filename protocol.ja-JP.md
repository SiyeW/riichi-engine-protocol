...
`major` が異なる場合は通信を継続してはいけません。同じ `major` では、ホストとエンジンは `0` から自身が宣言する最高の `minor` までをすべてサポートします。ホストは `engine.hello` リクエストで対応する最高の `minor` を提示し、エンジンは双方が対応する最高の `minor` を返します。返す値はホストの提示値を超えてはいけません。

`engine.hello` の結果とそれ以降のメッセージは、合意した `minor` に従う必要があります。エンジンは、それより新しい `minor` で追加されたフィールド、値、メソッド、機能を使用してはいけません。

プロトコルのバージョンが同じでも、双方が同じ出力に対応しているとは限りません。ホストは、自身が認識し、かつエンジンが宣言した出力だけを使用します。エンジンが追加の出力を宣言しても、ホストはそれを理由にエンジン全体を拒否してはいけません。
...
送信側は、確実に起こらない事象を正確な `0`、必ず起こる事象を正確な `1` で表すことができます。受信側はこれらの端点を近似値へ置き換えてはいけません。この規則は単独の確率、条件付き確率、離散分布の `probability` に適用されます。
...
牌種別の `tiles` には34種類の牌を使用し、赤五は区別しません。キー名は：

```text
1m..9m, 1p..9p, 1s..9s, E, S, W, N, P, F, C
```

完全な `tiles` には、この34個のキーを過不足なく含める必要があります。`5m`、`5p`、`5s` は、それぞれ同じ色の通常の五と赤五の合計です。枚数出力では、`5mr`、`5pr`、`5sr` をキーとする `redTiles` も出力できます。赤五の枚数は、対応する `tiles` の五にも含まれます。アクションデータの物理牌には赤五の表記を残すことができます。
...
| 出力契約ID | 意味 |
| --- | --- |
| `action-recommendation` | ホストが提示した合法手から1つを推薦し、必要に応じて各候補の評価指標も提供する。 |
| `opponent-shanten` | 他の座席の向聴分布や、聴牌の条件下での振聴または無役の確率を予測する。 |
| `opponent-deal-in-probability` | 各対戦相手に34種の牌を打ったときの制御された座席の放銃率を予測する。 |
| `opponent-concealed-tile-count` | 各対戦相手の暗牌における34種の牌を予測する。3種類の赤五は個別にも出力できる。 |
| `opponent-dora-count` | 各対戦相手のドラ数を予測する。 |
| `opponent-score` | 各対戦相手の打点を予測する。 |
...
`outputs` は、リクエスト内の各出力と同じ ID の結果を1つずつ返します。配列の順序にプロトコル上の意味はありません。初期化結果は、ハンドシェイクで宣言した表現、評価指標、明示手牌への対応を絞り込むことはできますが、能力を追加してはいけません。いずれかの出力を提供できない場合、初期化全体を失敗させ、出力を黙って削除してはいけません。
...
各対戦相手のシャンテン数分布と、テンパイ時にフリテンまたは役なしとなる確率を予測します。
...
操作対象の席が各種の牌を切ったとき、各対戦相手への放銃率を予測します。
...
`redTiles` は省略できます。出力する場合は `5mr`、`5pr`、`5sr` を過不足なく含め、離散分布の値は整数 `0..1`、`expectedValue` または `pointEstimate` は `[0, 1]` の範囲とします。

## `wall-tile-count`

未公開のまま牌山に残っている牌種ごとの枚数を予測します。
...
現在の局の流局率・和了率・放銃率、または排他的な最終結果分布を予測します。
...
現在の局面から局の精算が終わるまでの、4人の持ち点変化を予測します。
...
操作対象の席で選べるアクションについて、推奨結果と任意の評価指標を提供します。
...
- 実行に影響するエンジンパッケージ内のファイルや実行ファイルのダイジェスト；
- 使用中のすべての重みファイルのダイジェスト；
- 実際のデバイス種別、有効な設定、および数値精度に影響する実行設定；
- 出力契約 ID；
- 初期化時に確定した表現、指標 ID、指標形式、および優先方向；
- 結果の数値に影響するホスト側の後処理バージョン。
