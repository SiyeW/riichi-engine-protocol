...
数値予測では、次の2つの表現を使用できます。

| 表示 | 対応フィールド | 形式と意味 |
| --- | --- | --- |
| `distribution` | `distribution` | 離散確率分布。 |
| `expected-value` | `expectedValue` | 予測値の数学的期待値は、有限のJSON数値でなければなりません。 |

数値予測では、`distribution` の `value` は有限の JSON 数値でなければなりません。文字列の値は、他の離散分布を用いる出力契約でのみ使用され、`expectedValue` の計算には使用できません。

現在のエンジン構成では、初期化時に各出力がどの表現を使用するかが決定されます。宣言されたフィールドは、その後の各対応する結果に現れる必要があり、宣言されていないフィールドは現れてはいけません。もし両方の表現が同時に宣言されている場合、`expectedValue` は `1e-4` の絶対または相対許容差内で `distribution` の加重平均と等しくなければなりません。

数量分布の値は非負整数でなければなりません；打点分布の値は非負の整数点数でなければなりません。数量と打点の`expectedValue`は`0`より小さくてはならず、また実際に現れる可能性のある離散値である必要はありません。
...
| `representations` | 条件必須 | 数値予測で使用できる表現で、許容値は `distribution`、`expected-value` です。 |
...
`players` は「座席」節で定義された他の座席を使用する。各 `tiles` は正確に34種類の牌を含まなければならない。離散分布が存在する場合、値は `0..4` の整数でなければならず、未記載の値の確率は `0` である。ホストは分布から「少なくとも1枚持っている」確率 `1 - P(0)` を導出することができる。
...
`tiles` には34種の牌を過不足なく含める必要があります。離散分布を使用する場合、値は整数 `0..4` とし、記載されていない値の確率は `0` とします。
...
プロトコルが要求するのは、`players` が「座席」の節に従うこと、離散値が非負整数であること、`expectedValue` が `0` 以上であることだけです。エンジンが上記の統計的解釈を採用する必要はありません。この出力を別の意味のドラ数予測に使用しても、ホストは同じデータ構造として解析します。
