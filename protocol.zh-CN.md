...
发送方可以用精确的 `0` 和 `1` 分别表示已知不可能和必然发生的事件。接收方必须保留这两个端点，不得替换为近似值。此规则适用于单独概率、条件概率和离散分布中的 `probability`。
